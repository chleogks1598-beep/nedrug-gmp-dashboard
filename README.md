...
식약처가 **실사결과서가 아닌 잘못된 파일을 올리는 경우**가 있습니다(2026-09-02 중앙산업가스(주)
대덕 건은 문서보안 DRM 이 걸린 채로 업로드돼 PDF/HWPX 구조가 아예 없었습니다 — 접근 권한 문제가
아니라 식약처 쪽 오업로드입니다). 이런 건은 재시도로 풀리지 않으므로 위의 하드 스톱이 **열리지 않는
1건 때문에 같이 올라온 다른 신규 건과 이후 모든 갱신을 무기한 막습니다**(실제로 14시간 정지).
